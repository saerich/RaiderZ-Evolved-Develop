...
namespace SoulHunt
{
    ...
    public static class SHGlobal
    {
        ...
        public const String COK = "완료";
        public const String CERROR = "논리적인 오류를 가지고 있는 값이 존재합니다.";
        ...
        public static void Init()
        {
            ...
            if (!GetSetting("ProjectFolder", ref strProjectFolder))
            {
                ...
                {
                    MessageBox.Show("프로젝트 폴더가 설정되지 않았습니다. 이 프로그램을 사용하기 위해서는 반드시 올바른 프로젝트 폴더를 설정해야 합니다.", Application.ProductName);
                    Environment.Exit(-1);
                }
            }
            if (!IsSHFolder())
        ...
        {
            return File.Exists(strProjectFolder + "\\Raiderz.exe");
        }
        ...
        public static bool SetupFolder()
        {
            ...
            dlg.Description = "레이더즈 프로젝트 폴더를 설정해주십시오. 일반적으로 SVN Repository의 Raiderz Runtime을 설정합니다.";
...
//                Environment.Exit(-1);
        ...
        /// 좀 심각하게 엉터리로 짠 코드.
        /// </summary>
        /// <param name="szFileName">비교할 파일이름. 앞에 \는 생략된다.</param>
        ...
        {
            throw new Exception("XML 구문 에러: " + Path.GetFileName(e.Exception.SourceUri) + " [줄:" + e.Exception.LineNumber + ",열: " + e.Exception.LinePosition + "] " + e.Exception.Message);
        }
        ...
        private static void GrammarCheck(string szFileName, string szXsdName)
        {
            ...
            if (szXsdName != null && szXsdName != "")
            {
                ...
                {
                    throw new Exception("스키마 파일 에러: " + szXsdName + " [줄:" + ex.LineNumber + ",열:" + ex.LinePosition + "] " + ex.Message);
                }
                catch (ArgumentNullException ex)
                {
                    throw new Exception("스키마 파일 에러: " + ex.Message + " (잘못된 인자 전달로 추정. 프로그래머에게 문의바랍니다.)");
                }
            }
            ...
            {
                throw new Exception("XML 구문 에러: " + szFileName + " [줄:" + ex.LineNumber + ",열:" + ex.LinePosition + "] " + ex.Message);
            }
            finally
            {
                reader.Close();
        ...
        /// <summary>
        /// 파일을 체크한다. 만일 싱크되어야 하는 녀석이 발견되면 싱크를 하도록 만든다.
        /// </summary>
        public static void CheckFiles(BackgroundWorker bgWorker)
        {
            try
            {
                ...
                foreach (SHSmartCheckFile checkFile in _checkFiles)
                {
                    ...
                    {
                        if (bgWorker != null) bgWorker.ReportProgress(nCurrentProgress, Path.GetFileName(checkFile.FilePath) + " 동기화 체크");
                    ...
                    if (checkFile.CheckType == SHSCF_Type.Both || checkFile.CheckType == SHSCF_Type.Server)
                    {
                        ...
                        {
                            throw new Exception(GetServerSystemFolder() + checkFile.FilePath + "파일이 폴더에 없습니다.");
                        }
                        else
                        {
                            if (bgWorker != null) bgWorker.ReportProgress(nCurrentProgress, Path.GetFileName(checkFile.FilePath) + " 문법 체크");
                    ...
                    if (checkFile.CheckType == SHSCF_Type.Both || checkFile.CheckType == SHSCF_Type.Client)
                    {
                        ...
                        {
                            throw new Exception(GetClientSystemFolder() + checkFile.FilePath + "(c)파일이 폴더에 없습니다.");
                        }
                        else
                        {
                            if (bgWorker != null) bgWorker.ReportProgress(nCurrentProgress, Path.GetFileName(checkFile.FilePath) + "(c) 문법 체크");
                    ...
                    if (bgWorker != null) bgWorker.ReportProgress(nCurrentProgress, Path.GetFileName(checkFile.FilePath) + " 체크 완료");
                }
            }
            catch (Exception e)
            {
    ...
    #region 파일체크 관련(SHSCF_Type, SHSmartCheckFile)
