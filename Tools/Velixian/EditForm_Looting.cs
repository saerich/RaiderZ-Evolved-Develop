...
namespace Velixian
{
    public partial class EditForm_Looting : EditForm
    {
        ...
        private void ProcessLootSilver(ListViewItem lvi, SHLootSilver silver)
        {
            ...
            lvi.Text = "실버";
            lvi.SubItems.AddRange(new String[] { silver.rate.ToString() + "%", (silver.minSpecified) ? silver.min.ToString() : "(미정의)", (silver.maxSpecified) ? silver.max.ToString() : "(미정의)" });
            lvi.Tag = silver;
            lvi.ImageIndex = 9;
        ...
        private void ProcessLootItem(ListViewItem lvi, SHLootItem item)
        {
            ...
            lvi.Text = (xmlCore.Items.IsValid(item.id) && xmlCore.Items[item.id].Passed) ? xmlCore.GetSafeString(xmlCore.Items[item.id].name) : "미존재 아이템 (" + item.id.ToString() + ")";
            lvi.SubItems.AddRange(new String[] { item.rate.ToString() + "%", (item.minSpecified) ? item.min.ToString() : "(미정의)", (item.maxSpecified) ? item.max.ToString() : "(미정의)" });
            lvi.Tag = item;
            lvi.ImageIndex = 10;
        ...
        private void ProcessLootSoul(ListViewItem lvi, SHLootSoul soul)
        {
            ...
            lvi.Text = (soul.typeSpecified) ? "영혼 " + soul.type.ToString() + "랭크" : "(미정의)";
            lvi.SubItems.AddRange(new String[] { soul.rate.ToString() + "%", (soul.minSpecified) ? soul.min.ToString() : "(미정의)", (soul.maxSpecified) ? soul.max.ToString() : "(미정의)" });
            lvi.Tag = soul;
            lvi.ImageIndex = 11;
        ...
        {
            if (MessageBox.Show("정말 삭제하시겠습니까?", Application.ProductName, MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
        ...
        private void pgLoot_PropertyValueChanged(object s, PropertyValueChangedEventArgs e)
        {
            if (pgLoot.Tag.GetType() == typeof(ListViewItem))
            {
                ...
                if (loot != null)
                {
                    if (e.ChangedItem.Label == "(아이디)" && loot.id != (int)e.OldValue)
                    {
        ...
        private void lvLootList_DoubleClick(object sender, EventArgs e)
        {
            ...
            SHPropertyGridUtil.SelectGridItem(pgLootableDetail, "(확률)");
        ...
        {
            m_ListViewController.OnKeyDown(e, "전리품");
        }
