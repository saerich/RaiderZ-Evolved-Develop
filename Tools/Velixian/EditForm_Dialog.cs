    ...
    public partial class EditForm_Dialog : EditForm
    {
        ...
        private void subDialogDetailAdd_Click(object sender, EventArgs e)
        {
            ...
            if (nodeType == typeof(SHDialog))
            {
                ...
                {
                    MessageBox.Show("대화문에 연결된 SAY문은 한개만 올수 있습니다.", Application.ProductName);
                }
            }
        ...
        private void subDialogDetailDel_Click(object sender, EventArgs e)
        {
            ...
            if (nodeType == typeof(SHDialog))
            {
                MessageBox.Show("안전을 위해 대화문을 삭제할 수 없습니다. 왼쪽 대화목록에서 삭제하고 싶은 대화를 선택해서 삭제해 주십시오.", Application.ProductName);
                return;
            }
            ...
            if (nodeType == typeof(SHDialogSay))
            {
                ...
                if ((say.dataList.Count > 0 && MessageBox.Show("이 작업으로 하위 노드까지 전부 삭제됩니다. 계속하시겠습니까?", Application.ProductName, MessageBoxButtons.YesNo) == DialogResult.No)
                    || (say.dataList.Count == 0 && MessageBox.Show("정말 삭제하시겠습니까?", Application.ProductName, MessageBoxButtons.YesNo) == DialogResult.No)) return;
        ...
        {
            m_ListViewController.OnKeyDown(e, "대화");
        }
