...
bool MUnitTestConfig::Load()
{
	...
	if (GetPrivateProfileString(UNITTEST_CONFIG_TOKEN_APP_FILTER, "STAGE", "all", szValue, 256, filename))
	{
		...
		std::transform(m_strStageFilter.begin(), m_strStageFilter.end(), m_strStageFilter.begin(), [](unsigned char c) { return std::tolower(c); });
	}
...
bool MUnitTestConfig::CheckStageFilter( const char* szStageName )
{
	...
	std::transform(strName.begin(), strName.end(), strName.begin(), [](unsigned char c) { return std::tolower(c); });
