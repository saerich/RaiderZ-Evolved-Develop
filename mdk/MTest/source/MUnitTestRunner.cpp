...
bool MUnitTestRunner::CheckFilter( const UnitTest::TestDetails* details, Filter& filter )
{
	...
	for (size_t i = 0; i < filter.vstrSuiteAnyWords.size(); i++)
	{
		...
		std::transform(strSuiteNameLower.begin(), strSuiteNameLower.end(), strSuiteNameLower.begin(), [](unsigned char c) { return std::tolower(c); });
		std::transform(strAnyWordLower.begin(), strAnyWordLower.end(), strAnyWordLower.begin(), [](unsigned char c) { return std::tolower(c); });
