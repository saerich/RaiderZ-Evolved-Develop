...
{
	float yScale = 1.f/ static_cast<float>(tan(fFovY/2.f));
...
{
	float c = static_cast<float>(cos(radian));
	float s = static_cast<float>(sin(radian));
	...
	_11 = c+(1.f-c)*xx;
	_12 = (1.f-c)*xy + s*axis.z;
	_13 = (1.f-c)*xz - s*axis.y;
	_14 = 0.0f;

	_21 = (1.f-c)*xy - s*axis.z;
	_22 = c+(1.f-c)*yy;
	_23 = (1.f-c)*yz + s*axis.x;
	_24 = 0.0f;

	_31 = (1.f-c)*xz + s*axis.y;
	_32 = (1.f-c)*yz - s*axis.x; 
	_33 = c+(1.f-c)*zz;
	_34 = 0.0f;
...
inline void MMatrix::SetRotationYawPitchRoll(float yaw, float pitch, float roll)
{
	...
	float cp = static_cast<float>(cos(pitch));
	float sp = static_cast<float>(sin(pitch));
	float cr = static_cast<float>(cos(roll));
	float sr = static_cast<float>(sin(roll));
...
inline void MMatrix::SetLocalMatrix(const MVector3& position, const MVector3& dir, const MVector3& up)
{
	...
	{
		NRight = dir.CrossProduct(MVector3(0.f, 1.f, 0.f));
	}
...
{
	float c = static_cast<float>(cos(fRadian));
	float s = static_cast<float>(sin(fRadian));
