...
void __cdecl MLog0(const char *pFormat, ...)
{
	...
	if (MLogLevel::MLOG_LEVEL_DEBUG < g_DefaultLogger.GetLevel()) return;
	...
	g_DefaultLogger.Output(temp, MLogLevel::MLOG_LEVEL_DEBUG);
}
void __cdecl MLog1(const char *pFormat, ...)
{
	/// <중요> 변경할 때 ANSI 버전과 UNICODE 버전을 같이 작업해야한다. 
	if (MLogLevel::MLOG_LEVEL_INFORMATION < g_DefaultLogger.GetLevel()) return;
	...
	g_DefaultLogger.Output(temp, MLogLevel::MLOG_LEVEL_INFORMATION);
}
void __cdecl MLog2(const char *pFormat, ...)
{
	/// <중요> 변경할 때 ANSI 버전과 UNICODE 버전을 같이 작업해야한다. 
	if (MLogLevel::MLOG_LEVEL_WARNING < g_DefaultLogger.GetLevel()) return;
	...
	g_DefaultLogger.Output(temp, MLogLevel::MLOG_LEVEL_WARNING);
}
void __cdecl MLog3(const char *pFormat, ...)
{
	...
	g_DefaultLogger.Output(temp, MLogLevel::MLOG_LEVEL_ERROR);
}
void __cdecl MLog0(unsigned long int nOutputFlags, const char *pFormat, ...)
{
	...
	g_DefaultLogger.Output(nOutputFlags, temp, MLogLevel::MLOG_LEVEL_DEBUG);
...
void __cdecl MLog1(unsigned long int nOutputFlags, const char *pFormat, ...)
{
	...
	g_DefaultLogger.Output(nOutputFlags, temp, MLogLevel::MLOG_LEVEL_INFORMATION);
...
void __cdecl MLog2(unsigned long int nOutputFlags, const char *pFormat, ...)
{
	...
	g_DefaultLogger.Output(nOutputFlags, temp, MLogLevel::MLOG_LEVEL_WARNING);
}
void __cdecl MLog3(unsigned long int nOutputFlags, const char *pFormat, ...)
{
	...
	g_DefaultLogger.Output(nOutputFlags, temp, MLogLevel::MLOG_LEVEL_ERROR);
}
...
void __cdecl mlog_filter(const char* szFilter, const char* pFormat, ...)
{
	...
	g_DefaultLogger.Output(szFilter, temp, MLogLevel::MLOG_LEVEL_DEBUG);
}
...
void __cdecl mlog_tool(const char* pFormat, ...)
{
	...
	g_DefaultLogger.Output("tool", temp, MLogLevel::MLOG_LEVEL_INFORMATION);
}
...
MLogger::MLogger()
{
	...
	m_szXslPath[0] = 0;
	m_szLastError[0] = 0;
	m_nLevel = MLogLevel::MLOG_LEVEL_ERROR;
...
void MLogger::_OutputFile(const char* szTime, const char* szText, MLogLevel nLevel)
{
	...
	if (m_bPrintHeader)
	{
		switch (nLevel)
		{
		case MLogLevel::MLOG_LEVEL_DEBUG:			fprintf_s(pFile, "[d] %s - %s", szTime, MLocale::ConvAnsiToUTF8(szText).c_str()); break;
		case MLogLevel::MLOG_LEVEL_INFORMATION:	fprintf_s(pFile, "[i] %s - %s", szTime, MLocale::ConvAnsiToUTF8(szText).c_str()); break;
		case MLogLevel::MLOG_LEVEL_WARNING:		fprintf_s(pFile, "[w] %s - %s", szTime, MLocale::ConvAnsiToUTF8(szText).c_str()); break;
		case MLogLevel::MLOG_LEVEL_ERROR:			fprintf_s(pFile, "[e] %s - %s", szTime, MLocale::ConvAnsiToUTF8(szText).c_str()); break;
		}
	}
	else
	{
...
void MLogger::_OutputFile( const WCHAR* szTime, const WCHAR* szText, MLogLevel nLevel )
{
	...
	if (m_bPrintHeader)
	{
		switch (nLevel)
		{
		case MLogLevel::MLOG_LEVEL_DEBUG:			fprintf_s(pFile, "[d] %s - %s", MLocale::ConvUTF16ToUTF8(szTime).c_str(), MLocale::ConvUTF16ToUTF8(szText).c_str()); break;
		case MLogLevel::MLOG_LEVEL_INFORMATION:	fprintf_s(pFile, "[i] %s - %s", MLocale::ConvUTF16ToUTF8(szTime).c_str(), MLocale::ConvUTF16ToUTF8(szText).c_str()); break;
		case MLogLevel::MLOG_LEVEL_WARNING:		fprintf_s(pFile, "[w] %s - %s", MLocale::ConvUTF16ToUTF8(szTime).c_str(), MLocale::ConvUTF16ToUTF8(szText).c_str()); break;
		case MLogLevel::MLOG_LEVEL_ERROR:			fprintf_s(pFile, "[e] %s - %s", MLocale::ConvUTF16ToUTF8(szTime).c_str(), MLocale::ConvUTF16ToUTF8(szText).c_str()); break;
		}
	}
	else
	{
...
void MLogger::_OutputXml(const char* szTime, const char* szText, MLogLevel nLevel)
{
	...
	switch (nLevel)
	{
	case MLogLevel::MLOG_LEVEL_DEBUG:			strcpy_s(szAttrType, "debug"); break;
	case MLogLevel::MLOG_LEVEL_INFORMATION:	strcpy_s(szAttrType, "information"); break;
	case MLogLevel::MLOG_LEVEL_WARNING:		strcpy_s(szAttrType, "warning"); break;
	case MLogLevel::MLOG_LEVEL_ERROR:			strcpy_s(szAttrType, "error"); break;
	}
