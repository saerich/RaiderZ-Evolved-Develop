...
inline MQuat& MQuat::operator=(const MMatrix& other)
{
	...
	if (t > 0.0f)
	{
		l = static_cast<float>(sqrt(t + 1.0f));
		s = 0.5f / l;
		w = l * 0.5f;
	...
	}	else
	{
		...
		{
			case 0 :
				l = static_cast<float>(sqrt(1.0f + other._11 - other._22 - other._33));	// 이건 중복되는데, switch 위로 올리는게 나을듯.
				s = 0.5f / l;
				x = l * 0.5f;
			...
			case 1 :
				l = static_cast<float>(sqrt(1.0f - other._11 + other._22 - other._33));
				s = 0.5f / l;
...
{
	float fInvLength = 1.f/ static_cast<float>(sqrt(x * x + y * y + z * z + w * w));
	x *= fInvLength;
	y *= fInvLength;
	z *= fInvLength;
	w *= fInvLength;
...
{
	MVector3 result;
	double m00 = 1.0 - (2.0 * ((static_cast<double>(z)* static_cast<double>(z)) + static_cast<double>(y)* static_cast<double>(y)));
	double m01 = 2.0 * (static_cast<double>(x) * static_cast<double>(z )- static_cast<double>(w) * static_cast<double>(y));

	result.x = static_cast<float>(atan2( 2.0 * (static_cast<double>(y) * static_cast<double>(z) - static_cast<double>(w) * static_cast<double>(x)), 1.0 - (2.0 * ((static_cast<double>(x)* static_cast<double>(x)) + (static_cast<double>(z)* static_cast<double>(z))))));
	result.y = static_cast<float>(atan2(m01, m00));
	result.z = static_cast<float>(atan2(-2.0 * (static_cast<double>(x) * static_cast<double>(y) + static_cast<double>(w) * static_cast<double>(z)), sqrt((m00*m00) + (m01*m01))));
