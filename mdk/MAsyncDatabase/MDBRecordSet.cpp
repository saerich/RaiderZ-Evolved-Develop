...
namespace mdb
{
	...
	{
		MDBFieldValueType& fieldResult = FieldW(MLocale::ConvAnsiToUCS2(strName.c_str()));
			
		if (fieldResult.IsNull())
		{
			mlog3("MDBRecordSet::Field, Database record result does not contain column with name: \"%s\". Using default value. \n", strName.c_str());
		}
