...
MAP_EDIT_TYPE CCoreToolRS3Doc::OpenMapFile(const CString& sFile)
{
	...
	{
		CString message;
		message.Format("지원되지 않는 형식의 파일입니다. %s 파일 형식에 대응하는 Workspace가 있는지 확인하십시오.", strWorkSpaceKeyword.c_str());
...
int CCoreToolRS3Doc::ConfirmSaveAndClose(MAP_EDIT_TYPE& _maptype, LOAD_TYPE _type, const char* filename, const std::string& _rWorkSpaceKeyword)
{
	...
	else
	{
		iResult = MessageBox(NULL,
						"로드된 맵이 존재합니다. 저장하시겠습니까?",
						"경고 - 로드된 맵이 있습니다.", MB_YESNOCANCEL | MB_ICONASTERISK);
	}
...
void CCoreToolRS3Doc::OnSetReferenceMesh()
{
	...
	Dlg.GetOFN().lpstrTitle = _T("레퍼런스 메쉬 파일을 설정합니다.");
	Dlg.GetOFN().lpstrFilter = "elu파일(*.elu)\0*.elu";
