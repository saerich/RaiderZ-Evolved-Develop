...
void CMainFrame::OnClose()
{
	...
	{
		int iResult = MessageBox("수정된 사항을 저장하시겠습니까?", "경고", MB_YESNOCANCEL);
		switch ( iResult )
		{
		case IDCANCEL:
			return;
...
void CMainFrame::OnFileSaveall()
{
	CCoreToolRS3Doc* pDoc_ = static_cast<CCoreToolRS3Doc*>(GetActiveDocument());
	pDoc_->SaveMap();
	CStatusBarHandler::GetInstance()->ShowStatusBarMessage("맵이 저장되었습니다.");
}
...
void CMainFrame::OnObjImport()
{
	...
	Dlg.GetOFN().lpstrTitle = _T(".obj파일을 선택하세요");
	Dlg.GetOFN().lpstrFilter = "obj파일(*.obj)\0*.obj";
