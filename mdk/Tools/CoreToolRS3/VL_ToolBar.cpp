...
//	CInitDialogBar::OnInitDialogBar();
	...
	m_ctlToolTip_.AddTool(GetDlgItem(IDC_TOP_FILE_NEW), _T("새 파일"));
	m_ctlToolTip_.AddTool(GetDlgItem(IDC_TOP_FILE_OPEN), _T("파일열기(Ctrl + O)"));
	m_ctlToolTip_.AddTool(GetDlgItem(IDC_TOP_FILE_SAVE), _T("저장하기(Ctrl + S)"));
	m_ctlToolTip_.AddTool(GetDlgItem(IDC_TOP_CMD_UNDO), _T("언두(언두목록:F1키로 확인가능)"));

	m_ctlToolTip_.AddTool(GetDlgItem(IDC_TOP_HEIGH), _T("지형 높낮이(H)"));
	m_ctlToolTip_.AddTool(GetDlgItem(IDC_TOP_AVER), _T("지형 평균브러쉬(G)"));
	m_ctlToolTip_.AddTool(GetDlgItem(IDC_TOP_FLAT), _T("지형 평평하게(V)"));
	m_ctlToolTip_.AddTool(GetDlgItem(IDC_TOP_SPLAT), _T("지형 스플래팅(B)"));
	m_ctlToolTip_.AddTool(GetDlgItem(IDC_TOP_RELAX), _T("지형 완만하게(Relax)(L)"));
	m_ctlToolTip_.AddTool(GetDlgItem(IDC_TOP_GRASS), _T("지형에 풀심기(N)"));

	m_ctlToolTip_.AddTool(GetDlgItem(IDC_TOP_OBJ_NEW), _T("새로운 오브젝트"));
	m_ctlToolTip_.AddTool(GetDlgItem(IDC_TOP_SELECT), _T("오브젝트 선택(T)"));
	m_ctlToolTip_.AddTool(GetDlgItem(IDC_TOP_MOVE), _T("오브젝트 이동(Q)"));
	m_ctlToolTip_.AddTool(GetDlgItem(IDC_TOP_ROTATE), _T("오브젝트 회전(R)"));
	m_ctlToolTip_.AddTool(GetDlgItem(IDC_TOP_SCALE), _T("오브젝트 크기편집(E)"));

	m_ctlToolTip_.AddTool(GetDlgItem(IDC_AXIS_X), _T("X축 편집(F5)"));
	m_ctlToolTip_.AddTool(GetDlgItem(IDC_AXIS_Y), _T("Y축 편집(F6)"));
	m_ctlToolTip_.AddTool(GetDlgItem(IDC_AXIS_Z), _T("Z축 편집(F7)"));
	m_ctlToolTip_.AddTool(GetDlgItem(IDC_AXIS_XYZ), _T("멀티 축 편집(F8)"));
	m_ctlToolTip_.AddTool(GetDlgItem(IDC_AXIS_LOCAL), _T("로컬축 편집"));

	m_ctlToolTip_.AddTool(GetDlgItem(IDC_TOP_WEATHER), _T("날씨 정보보기"));
	m_ctlToolTip_.AddTool(GetDlgItem(IDC_TOP_OBJECT_LIST), _T("씬 오브젝트 리스트"));
	m_ctlToolTip_.AddTool(GetDlgItem(IDC_TOP_LAYER_MANAGER), _T("레이어 매니져"));

	m_ctlToolTip_.AddTool(GetDlgItem(IDC_TOP_OBJ_ON_TERRAIN), _T("선택한 물체 지형에 붙이기(Z)"));
	m_ctlToolTip_.AddTool(GetDlgItem(IDC_TOP_LOCK), _T("물체선택 잠금 - 선택한 물체 편집(C)"));

	m_ctlToolTip_.AddTool(GetDlgItem(IDC_TOP_PATH_FIND), _T("길찾기"));
	m_ctlToolTip_.AddTool(GetDlgItem(IDC_TOP_ETC_MAP_VIEW), _T("맵 정보"));
	m_ctlToolTip_.AddTool(GetDlgItem(IDC_TOP_ETC_DEV), _T("개발자용"));
	m_ctlToolTip_.AddTool(GetDlgItem(IDC_TOP_CLIP), _T("클리핑"));
