...
void XReplayAutoTest::End()
{
	...
	if (global.app) 
	{
		int nAppRet = MUnitTestRunner::CCNET_UNITTEST_SUCCESS;

		// 차후 목표 fps를 exe 파라메타에서 얻도록 수정한다.
		if (m_Data.GetAverageFPS() < m_Baseline.nFPS) nAppRet = MUnitTestRunner::CCNET_UNITTEST_FAILED;

		global.app->Quit(nAppRet);
	}
}
...
void XReplayAutoTest::Fail( const wchar_t* szMsg )
{
	...
	{
		OutputReport();
		global.app->Quit(MUnitTestRunner::CCNET_UNITTEST_FAILED);
	}
}
