...
void XGlueGame::LoginConnect( const char* szServer)
{	
	gvar.MyInfo.strLoginID = gvar.Network.szLoginID;
	if ( global.net)		global.net->PostConnectToLoginServer( MLocale::ConvAnsiToUTF16( szServer).c_str());
}
...
void XGlueGame::CreateNewCharacter( const char* szName, MWLua::table t)
{
	...
	{
		TALENT_STYLE __STYLE[] = { TALENT_STYLE_DEFENDER, TALENT_STYLE_BERSERKER, TALENT_STYLE_CLERIC };
...
void XGlueGame::LootAllItems()
{
	...
	for ( size_t i = 0;  i < _size;  i++)
	{
		...
		if(pItemData->m_bUnique == true)
		{
			...
			{
				global.ui->OnSystemMsg(L"{0}를(을) 가지고 있습니다.", FSParam(pItemData->GetName()));
				continue;
			}
		}
...
{
	wstring strPlayerName = XCommonFunction::GetPlayerName( XGetInteractionInfo().InteractionTargetUID);
	global.ui->OnSystemMsg(L"{0}님께 결투를 신청 하였습니다.", FSParam(strPlayerName));
...
{
	wstring strPlayerName = XCommonFunction::GetPlayerName( XGetInteractionInfo().InteractionTargetUID);
	global.ui->OnSystemMsg(L"{0}님께 파티결투를 신청 하였습니다.", FSParam(strPlayerName));
...
const char* XGlueGame::GetMailErrorMsg( int _errcode)
{
	...
	case CR_FAIL_MAIL_POST_SELF :				gvar.Temp.szTemp = "자기 자신에게는 편지를 보낼 수 없습니다.";				break;
	case CR_FAIL_MAIL_POST_LIMIT_TITLE :		gvar.Temp.szTemp = "편지 제목이 너무 깁니다.";								break;
	case CR_FAIL_MAIL_POST_LIMIT_TEXT :			gvar.Temp.szTemp = "편지 본문 내용이 너무 깁니다.";							break;
	case CR_FAIL_MAIL_POST_NOT_FIND_USER :		gvar.Temp.szTemp = "편지를 받는 사람을 확인할 수 없습니다.";				break;
	case CR_FAIL_MAIL_POST_NOT_SEND_GM :		gvar.Temp.szTemp = "운영자에게는 편지를 보낼 수 없습니다.";					break;
	case CR_FAIL_MAIL_POST_FULL_BOX :			gvar.Temp.szTemp = "편지를 받는 사람의 우편함이 가득 차있어서 편지를 보낼 수 없습니다.";	break;
	case CR_FAIL_MAIL_POST_NOT_ENOUGH_POSTAGE :	gvar.Temp.szTemp = "소지금이 부족하여 편지를 보낼 수 없습니다.";			break;
	default :									gvar.Temp.szTemp = "편지를 보낼 수 없습니다.";								break;
	}
