...
void FSContext::CastingState(wchar_t token)
{
	...
	case L'\xc744':
	case L'\xb97c':
		if (HasJonsung())	m_strPostfix.push_back(L'\xc744');
		else m_strPostfix.push_back(L'\xb97c');
	...
	case L'\xc774':
	case L'\xac00':
		if (HasJonsung())	m_strPostfix.push_back(L'\xc774');
		else m_strPostfix.push_back(L'\xac00');
	...
	case L'\xc740':
	case L'\xb294':
		if (HasJonsung())	m_strPostfix.push_back(L'\xc740');
		else m_strPostfix.push_back(L'\xb294');
