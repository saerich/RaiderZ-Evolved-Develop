...
void SCommonBodyCharGetLookList::GetCharSimpleInfo(mdb::MDBRecordSet& rs)
{
	...
	for (m_Data.nCharCount = 0; !rs.IsEOF(); rs.MoveNext(), ++m_Data.nCharCount)
	{
		...
		Char.nTattooType											= rs.Field("TATTOO_TYPE").AsByte();
		Char.nTattooPosX											= rs.Field("TATTOO_POS_X").AsShort();
		Char.nTattooPosY											= rs.Field("TATTOO_POS_Y").AsShort();
		Char.nTattooScale											= rs.Field("TATTOO_SCALE").AsByte();
