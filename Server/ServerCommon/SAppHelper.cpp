...
{	
	MInitLog(MLogLevel::MLOG_LEVEL_DEBUG, MLOG_DEBUG | MLOG_FILE, NULL, MLocale::ConvTCHARToAnsi(szFileName).c_str());

	MLogLevel nLogLevel = MLogLevel::MLOG_LEVEL_INFORMATION;

#ifndef _PUBLISH
	nLogLevel = MLogLevel::MLOG_LEVEL_DEBUG;
#	ifndef _DEBUG
...
void SAppHelper::OutputLogToConsole( MLogLevel nLevel, const wchar_t* szText )
{
	switch (nLevel)
	{
	case MLogLevel::MLOG_LEVEL_DEBUG:			SAppHelper::ColoredPrintf(SAppHelper::SConsoleColor_Default, szText); break;
	case MLogLevel::MLOG_LEVEL_INFORMATION:	SAppHelper::ColoredPrintf(SAppHelper::SConsoleColor_White, szText); break;
	case MLogLevel::MLOG_LEVEL_WARNING:		SAppHelper::ColoredPrintf(SAppHelper::SConsoleColor_Yellow, szText); break;
	case MLogLevel::MLOG_LEVEL_ERROR:
		{
...
void SAppHelper::PostLogOutputCommand( SCommandCenter* pCommandCenter, MUID uidServer, const wchar_t* szText, MLogLevel nLevel )
{
	...
	pNewCmd->AddParameter(NEW_INT(static_cast<int>(nLevel)));
