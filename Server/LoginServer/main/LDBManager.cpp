...
bool LDBManager::LoginGetInfo(LDBT_ACC_LOGIN& data)
{
	...
	CString strSQL;
	strSQL.Format(g_szDB_LOGIN_GETINFO, data.m_strUSER_ID.c_str(), data.m_strPWD.c_str());
...
//
//	CString strSQL;
//	strSQL.Format(g_szDB_PMANG_LOGIN_GETINFO, pmangUserData.nUserType , pmangUserData.strUserSerial.c_str());
//
...
//
//	CString strSQL;
//	strSQL.Format(g_szDB_PMANG_ACC_INSERT, pmangUserData.strUserID.c_str(), pmangUserData.nUserType, pmangUserData.strUserSerial.c_str(), L"");
//
...
bool LDBManager::ConnectLog( LDBT_CONN_LOG& data )
{
	...
	strSQL.Format(L"{CALL dbo.USP_RZ_CONN_LOG (%I64d, %d, '%s')}"
		, data.m_nAID, LConfig::m_nWorldID, data.m_strIP.c_str());
