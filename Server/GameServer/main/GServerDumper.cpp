...
{	
	if (pLogger == NULL)
	{
		...
		pLogger = &m_Logger;
		pLogger->Init(MLogLevel::MLOG_LEVEL_DEBUG, MLOG_FILE, MLocale::ConvUTF16ToAnsi(strFileName.c_str()).c_str(), NULL, NULL);
...
void GServerDumper::Log( MLogger* pLogger, const wchar_t* pFormat, ... )
{
	...
	pLogger->Output(buf, MLogLevel::MLOG_LEVEL_INFORMATION);
