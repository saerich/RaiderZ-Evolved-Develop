...
{
	MGetDefaultLogger()->Output(MLOG_FILE, szStr, MLogLevel::MLOG_LEVEL_DEBUG);
	mlog(szStr);
	GAIMonitorPort::Inst().SendGameLogToViewer(MLogLevel::MLOG_LEVEL_DEBUG, szStr);
}
...
{
	mlog1(szStr);
	GAIMonitorPort::Inst().SendGameLogToViewer(MLogLevel::MLOG_LEVEL_INFORMATION, szStr);
}

void GLuaGlues::GLog2(const char* szStr)
{
	mlog2(szStr);
	GAIMonitorPort::Inst().SendGameLogToViewer(MLogLevel::MLOG_LEVEL_WARNING, szStr);
}

void GLuaGlues::GLog3(const char* szStr)
{
	mlog3(szStr);
	GAIMonitorPort::Inst().SendGameLogToViewer(MLogLevel::MLOG_LEVEL_ERROR, szStr);
}
